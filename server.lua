...
require 'modules.asc_staff_blacklist.server'
...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		...
		local function notifyEsx(target, msg)
		...
		if not success then
			if actorSource and actorSource > 0 then
				notifyEsx(actorSource, ('~r~Give échoué: %sx %s (%s)'):format(count, item.name, response or 'error'))
			end
			return Citizen.Trace(('Failed to give %sx %s to player %s (%s)'):format(count, item.name, args.target, response))
		end

		notifyEsx(args.target, ('~g~Tu as reçu %sx %s.'):format(count, item.name))
		if actorSource and actorSource > 0 then
			notifyEsx(actorSource, ('~g~Give réussi: %sx %s -> ID %s'):format(count, item.name, args.target))
		end
...
lib.addCommand('removeitem', {
	...
	if item and args.count > 0 then
		...
		if not success then
			if actorSource and actorSource > 0 then
				notifyEsx(actorSource, ('~r~Remove échoué: %sx %s (%s)'):format(args.count, item.name, response or 'error'))
			end
			return Citizen.Trace(('Failed to remove %sx %s from player %s (%s)'):format(args.count, item.name, args.target, response))
		end

		notifyEsx(args.target, ('~o~%sx %s retiré(s) de ton inventaire.'):format(args.count, item.name))
		if actorSource and actorSource > 0 then
			notifyEsx(actorSource, ('~g~Remove réussi: %sx %s <- ID %s'):format(args.count, item.name, args.target))
		end
