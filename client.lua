...
local AscensionClone = require 'modules.ascension_clone.client'
local Weapon = require 'modules.weapon.client'

--- Gel du ped pendant l’inventaire (évite décalage clone / tenue). Désactivable : `setr inventory:ascension_freeze_player 0`
local ascensionInvFreezeActive = false

local function ascensionFreezePlayerWhileInventory(open)
	if GetConvarInt('inventory:ascension_freeze_player', 1) ~= 1 then return end
	local ped = cache.ped
	if not ped or ped == 0 or not DoesEntityExist(ped) then return end
	if open then
		if ascensionInvFreezeActive then return end
		FreezeEntityPosition(ped, true)
		ascensionInvFreezeActive = true
	else
		if not ascensionInvFreezeActive then return end
		FreezeEntityPosition(ped, false)
		ascensionInvFreezeActive = false
	end
end
local currentWeapon
...
function client.openInventory(inv, data)
	...
	ascensionFreezePlayerWhileInventory(true)
	...
	SetTimeout(50, function()
		if plyState.invOpen then AscensionClone.start() end
	end)
	...
	if item and usingItem then
		...
		if success then
			if item.notification then
				local message = tostring(item.notification)
				if GetResourceState('esx_notify') == 'started' then
					exports['esx_notify']:Notify('info', 3500, message, 'Inventaire', nil)
				else
...
RegisterNetEvent('asc_staff:client:oxInventoryPointsApply', function(data)
	if not data then return end
	--- nil = ne pas modifier (ex. sync blacklist seule au démarrage ox_inventory sans asc_staff)
	if data.shops ~= nil then
		Shops.mergeAscensionShops(data.shops)
	end
	if data.disabledBuiltinShops ~= nil and Shops.applyBuiltinShopBlacklist then
		Shops.applyBuiltinShopBlacklist(data.disabledBuiltinShops)
	end
	if data.crafting ~= nil then
		TriggerEvent('asc_staff:oxInventory:applyCrafting', data.crafting)
	end
	if data.disabledBuiltinCrafting ~= nil then
		TriggerEvent('asc_staff:oxInventory:syncBuiltinCraftingClient', data.disabledBuiltinCrafting)
	end
end)

CreateThread(function()
	Wait(4500)
	TriggerServerEvent('ox_inventory:server:requestBuiltinBlacklist')
end)
...
local AscensionClothingMemory = {}
--- Après retrait sur un slot (prop ou composant), délai avant de pouvoir réequiper sur **ce** slot uniquement.
local AscensionClothingCooldown = {}
local ASC_CLOTHING_COOLDOWN_MS = 5000

local function clearAscensionClothingMemory()
	for k in pairs(AscensionClothingMemory) do
		AscensionClothingMemory[k] = nil
	end
	for k in pairs(AscensionClothingCooldown) do
		AscensionClothingCooldown[k] = nil
	end
end

--- Notifications tenue / slots Ascension → `esx_notify` si dispo (sinon ox_lib).
---@param description string
---@param notifType? 'success'|'error'|'inform'
local function ascensionNotifyClothing(description, notifType)
	notifType = notifType or 'inform'
	local esxType = notifType == 'error' and 'error' or (notifType == 'success' and 'success' or 'info')
	local length = 4500
	if GetResourceState('esx_notify') == 'started' then
		exports['esx_notify']:Notify(esxType, length, tostring(description or ''), 'Inventaire', nil)
	else
		lib.notify({ type = notifType, description = description })
	end
end

--- Millisecondes restantes avant de pouvoir réequiper sur ce slot (0 = ok).
local function ascensionClothingCooldownLeftMs(slotKey)
	local untilAt = AscensionClothingCooldown[slotKey]
	if not untilAt then return 0 end
	local now = GetGameTimer()
	if now >= untilAt then
		AscensionClothingCooldown[slotKey] = nil
		return 0
	end
	return untilAt - now
end

local function ascensionClothingBeginCooldown(slotKey)
	AscensionClothingCooldown[slotKey] = GetGameTimer() + ASC_CLOTHING_COOLDOWN_MS
end

local function ascensionClothingClearCooldown(slotKey)
	AscensionClothingCooldown[slotKey] = nil
end

local function ascensionApplyPedClothingSaved(ped, saved)
	if not ped or ped == 0 or not saved then return end
	local illenium = GetResourceState('illenium-appearance') == 'started'
	if saved.kind == 'prop' then
		SetPedPropIndex(ped, saved.id, saved.d, saved.t, false)
		if illenium then
			pcall(function()
	...
	elseif saved.kind == 'comp' then
		if not IsPedComponentVariationValid(ped, saved.id, saved.d, saved.t) then return end
		SetPedComponentVariation(ped, saved.id, saved.d, saved.t, 0)
		if illenium then
			pcall(function()
...
--- Fermeture inventaire : on ne **réapplique pas** tout seul la mémoire (sinon ça annule les retraits).
--- On **conserve** AscensionClothingMemory : en rouvrant l’inventaire, un reclic sur chaque slot concerné
--- remet la pièce sauvegardée (comportement « retirer / fermer / rouvrir / reclic pour remettre »).
local function onInventoryCloseAscensionClothing()
	local ped = cache.ped
	if not ped or ped == 0 then
		return clearAscensionClothingMemory()
	end
	--- Aligne illenium sur le ped déjà corrigé par les natives.
	--- Ne pas utiliser setPlayerAppearance : il appelle setPlayerModel + SetPedDefaultComponentVariation,
	--- ce qui peut laisser le perso « nu » si la réapplication échoue ou est désynchronisée à la fermeture NUI.
	if GetResourceState('illenium-appearance') == 'started' then
		pcall(function()
			local app = exports['illenium-appearance']:getPedAppearance(ped)
			if app then
				exports['illenium-appearance']:setPedAppearance(ped, app)
			end
		end)
	end
end
...
function client.closeInventory(server)
	...
	if invOpen then
		onInventoryCloseAscensionClothing()
		AscensionClone.stop()
		ascensionFreezePlayerWhileInventory(false)
		invOpen = nil
	...
	if owner == cache.serverId then
		...
		if invOpen and #(GetEntityCoords(playerPed) - data.coords) <= 1 then
			if not cache.vehicle then
				...
				SetTimeout(100, function()
					if plyState.invOpen then AscensionClone.sync() end
				end)
			end
		end
	end
...
--- Molette sur le trou : rotation **uniquement** du clone (aperçu), pas du joueur.
RegisterNUICallback('ascensionRotatePed', function(data, cb)
	cb(1)
	if not plyState.invOpen then return end
	local delta = tonumber(data and data.delta)
	if not delta or delta == 0 then return end
	AscensionClone.rotateWithPlayer(delta)
end)

--- Centre normalisé du viewport transparent (NUI) → aligne le clone sur le « trou » Ascension
RegisterNUICallback('ascensionCloneViewport', function(data, cb)
	cb(1)
	local sx = data and tonumber(data.screenX)
	local sy = data and tonumber(data.screenY)
	if sx and sy then
		AscensionClone.setScreenTarget(sx, sy)
	end
end)

--- Clic slot vêtement Ascension : 1) item `clothing` → useSlot ; 2) sinon si mémoire → remettre ; 3) sinon retirer et mémoriser pour reclic.
RegisterNUICallback('ascensionEquipClothing', function(data, cb)
	cb(1)
	if not plyState.invOpen or usingItem or invBusy then return end

	local ped = cache.ped
	if not ped or ped == 0 then return end

	local wantProp = data and data.prop
	local wantComp = data and data.component
	if wantProp == nil and wantComp == nil then return end

	local propId = wantProp ~= nil and tonumber(wantProp) or nil
	local compId = wantComp ~= nil and tonumber(wantComp) or nil
	local slotKey = propId and ('p' .. propId) or ('c' .. compId)

	local function metaMatches(metadata)
		local drawable = tonumber(metadata.drawable)
		local texture = tonumber(metadata.texture)
		if not drawable or not texture then return false end
		if wantProp ~= nil then
			return tonumber(metadata.prop) == tonumber(wantProp)
		end
		return tonumber(metadata.component) == tonumber(wantComp)
	end

	for slot, item in pairs(PlayerData.inventory) do
		if type(item) == 'table' and item.name == 'clothing' and item.count and item.count > 0 and item.metadata and metaMatches(item.metadata) then
			local cd = ascensionClothingCooldownLeftMs(slotKey)
			if cd > 0 then
				local sec = math.max(1, math.ceil(cd / 1000))
				ascensionNotifyClothing(('Patiente %d s avant de remettre une pièce sur ce slot.'):format(sec), 'inform')
				return
			end
			AscensionClothingMemory[slotKey] = nil
			ascensionClothingClearCooldown(slotKey)
			useSlot(tonumber(slot) or slot)
			SetTimeout(200, function()
				if plyState.invOpen then AscensionClone.sync() end
			end)
			return
	...
	local saved = AscensionClothingMemory[slotKey]
	if saved then
		local cd = ascensionClothingCooldownLeftMs(slotKey)
		if cd > 0 then
			local sec = math.max(1, math.ceil(cd / 1000))
		...
		if saved.kind == 'comp' and not IsPedComponentVariationValid(ped, saved.id, saved.d, saved.t) then
			AscensionClothingMemory[slotKey] = nil
			AscensionClone.sync()
			return ascensionNotifyClothing('Impossible de remettre cette pièce sur ce modèle.', 'error')
		end
		ascensionApplyPedClothingSaved(ped, saved)
		AscensionClothingMemory[slotKey] = nil
		ascensionClothingClearCooldown(slotKey)
		AscensionClone.sync()
		return ascensionNotifyClothing('Tenue remise sur ce slot.', 'success')
	end

	local illenium = GetResourceState('illenium-appearance') == 'started'
	local model = GetEntityModel(ped)
	local male = model == `mp_m_freemode_01`
	local female = model == `mp_f_freemode_01`
	local stripDrawable = male and {
		[1] = { 0, 0 },
		[3] = { 15, 0 },
		[4] = { 61, 0 },
		[5] = { 0, 0 },
	...
	local function trySetComponent(cid, drawable, texture)
		if not cid or cid == 0 or cid == 2 then return false end
		if not IsPedComponentVariationValid(ped, cid, drawable, texture) then return false end
		SetPedComponentVariation(ped, cid, drawable, texture, 0)
		if illenium then
			pcall(function()
				exports['illenium-appearance']:setPedComponent(ped, {
					component_id = cid,
	...
	if propId ~= nil then
		if GetPedPropIndex(ped, propId) == -1 then
			return ascensionNotifyClothing('Aucun accessoire sur ce slot.', 'inform')
		end
		AscensionClothingMemory[slotKey] = {
			kind = 'prop',
			id = propId,
			d = GetPedPropIndex(ped, propId),
			t = GetPedPropTextureIndex(ped, propId),
		}
		...
		ascensionClothingBeginCooldown(slotKey)
		AscensionClone.sync()
		return ascensionNotifyClothing('Accessoire retiré (reclic pour remettre).', 'success')
	end

	if compId then
		local curD = GetPedDrawableVariation(ped, compId)
		local curT = GetPedTextureVariation(ped, compId)
		local pair = stripDrawable[compId]
		local targetD, targetT = 0, 0
		if pair then
			targetD, targetT = pair[1], pair[2]
		end
		local alreadyMinimal = pair and (curD == targetD and curT == targetT) or (not pair and curD == 0 and curT == 0)
		if alreadyMinimal then
			return ascensionNotifyClothing('Ce slot est déjà au minimum.', 'inform')
		end

		AscensionClothingMemory[slotKey] = { kind = 'comp', id = compId, d = curD, t = curT }

		if pair and trySetComponent(compId, targetD, targetT) then
			ascensionClothingBeginCooldown(slotKey)
			AscensionClone.sync()
			return ascensionNotifyClothing('Tenue retirée sur ce slot (reclic pour remettre).', 'success')
		end

		if trySetComponent(compId, 0, 0) then
			ascensionClothingBeginCooldown(slotKey)
			AscensionClone.sync()
			return ascensionNotifyClothing('Tenue mise à jour (reclic pour remettre).', 'success')
		end

		AscensionClothingMemory[slotKey] = nil
	end

	ascensionNotifyClothing('Pas d\'item dans l\'inventaire et impossible de retirer ce slot sur ce modèle.', 'inform')
end)
...
RegisterNUICallback('useItem', function(slot, cb)
	...
	local ok, err = pcall(useSlot, s)
	if not ok then
		warn(('ox_inventory NUI useItem (slot=%s): %s'):format(tostring(s), tostring(err)))
	end
	cb(1)
end)
