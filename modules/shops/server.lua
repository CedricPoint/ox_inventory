...
local shopsDataFile = lib.load('data.shops') or {}
local builtinShopTypeList = {}
for shopType in pairs(shopsDataFile) do
	builtinShopTypeList[#builtinShopTypeList + 1] = shopType
end
table.sort(builtinShopTypeList)

for i = 1, #builtinShopTypeList do
	local k = builtinShopTypeList[i]
	registerShopType(k, table.deepclone(shopsDataFile[k]))
end

--- Réapplique les boutiques data/shops.lua : désactivées retirées, les autres depuis une copie propre
exports('SyncBuiltinShopsAfterBlacklist', function(disabledShopKeys)
	local dis = {}
	if type(disabledShopKeys) == 'table' then
		for i = 1, #disabledShopKeys do
			local k = disabledShopKeys[i]
			if type(k) == 'string' then dis[k] = true end
		end
	end
	for i = 1, #builtinShopTypeList do
		local k = builtinShopTypeList[i]
...
--- Liste des boutiques définies dans data/shops.lua (clés + nom affiché)
exports('GetBuiltinShopTypes', function()
	local list = {}
	for i = 1, #builtinShopTypeList do
		local k = builtinShopTypeList[i]
		local d = shopsDataFile[k]
		list[i] = { key = k, name = d and d.name or k }
	end
	return list
end)
...
--- Retire une boutique du registre serveur (défaut data/shops ou dynamique)
exports('UnregisterShop', function(shopType)
	if type(shopType) ~= 'string' then return end
	Shops[shopType] = nil
end)

--- Supprime les boutiques dynamiques asc_staff (clés ASC_OX_*) avant réapplication SQL
exports('ClearAscensionDynamicShops', function()
	for shopType in pairs(Shops) do
		if type(shopType) == 'string' and shopType:sub(1, 7) == 'ASC_OX_' then
			Shops[shopType] = nil
		end
	end
end)
