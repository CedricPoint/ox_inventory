...
-- qbx_vehicles : optionnel sur une base minimale (sinon assert = échec démarrage + chargement infini)
lib.checkDependency('qbx_vehicles', '1.2.0', true)
...
function server.getOwnedVehicleId(entityId)
    local vid = Entity(entityId).state.vehicleid
    if vid then return vid end
    if GetResourceState('qbx_vehicles') ~= 'started' then return end
    return exports.qbx_vehicles:GetVehicleIdByPlate(GetVehicleNumberPlateText(entityId))
end
