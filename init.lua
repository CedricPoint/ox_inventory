...
-- Client : sans setr, inventory:framework reste « esx » par défaut → pont ESX alors que le serv est Qbox
if not IsDuplicityVersion()
    and shared.framework == 'esx'
    and GetResourceState('qbx_core'):find('start')
    and GetResourceState('es_extended') ~= 'started'
then
    shared.framework = 'qbx'
    warn('^3[ox_inventory]^0 Utilisez setr inventory:framework "qbx" dans ox.cfg (convar répliquée client). Correction automatique : qbx.')
end
...
if IsDuplicityVersion() then
    ...
    client = {
        ...
        -- Ascension : 0 par défaut pour voir le perso net (convar inventory:screenblur pour réactiver le flou vanilla)
